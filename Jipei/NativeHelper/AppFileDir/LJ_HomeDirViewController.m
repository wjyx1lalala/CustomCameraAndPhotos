...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([[infoDict objectForKey:NSFileType] isEqualToString:NSFileTypeDirectory] == NO) {
        ...
        if ([infoDict[@"FileType"] hasPrefix:@"image"]) {
            ...
            //[[UIImage alloc] initWithContentsOfFile:[self.filePath stringByAppendingPathComponent:cell.textLabel.text]];
        }else{
            //unknow_icon  未知文件类型
            cell.imageView.image = [UIImage imageNamed:@"lj_unknow_icon.jpeg"];
        }
    }else{
        //文件夹模式
