...
#define BOUNDCE_DURATION 0.4f //移动后,回到原始位置的动画时间
...
- (instancetype)initWithImage:(UIImage *)originalImage{
  ...
  [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault];
    const CGFloat marginTop = ([UIScreen mainScreen].bounds.size.height - [UIScreen mainScreen].bounds.size.width - 64)/2;
    self = [super init];
    if (self) {
        self.cropFrame = CGRectMake(0, marginTop + 64, [UIScreen mainScreen].bounds.size.width , [UIScreen mainScreen].bounds.size.width);
        self.limitRatio = 3;
...
- (void)clickOk{
    //按钮高亮时  title颜色 213 213 213
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(ImageCropperDelegate)]) {
        [self.delegate imageCropperDidFinished:[self getSubImage]];
    }
}
...
- (void)initView {
    ...
    UIBarButtonItem *okBtn = [[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(clickOk)];
    ...
    CGFloat oriHeight = oriWidth * ( self.originalImage.size.height / self.originalImage.size.width);
    //起点坐标
